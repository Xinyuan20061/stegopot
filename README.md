...
- **独立可视化工作台**：Console 通过脱敏 JSON 契约展示拓扑、消息、检测和研究真值。
...
| 实验可视化界面 | 可用 | `stegopot-console/` 提供隔离的报告 API、拓扑和交互记录工作台 |
| 分布式执行 | 尚未实现 | 当前运行器仍为单进程同步调度 |
...
Python 包目录只保留四个逻辑层。依赖只能沿箭头方向流动：`domain` 保存模型和
...
| `stegopot-console` | 仓库级展示子项目 | 通过版本化 JSON 契约读取报告，不导入业务包 |
